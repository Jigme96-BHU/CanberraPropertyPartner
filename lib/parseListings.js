...
export async function parseREAXML(xmlString, options = {}) {
  if (!xmlString) return [];
  ...
  try {
    ...
    if (propertyList.rental) {
      ...
      items.forEach(item => {
        // Skip non-current listings when currentOnly is true
        if (currentOnly && item.status !== 'current') return;
...
function parseRental(item) {
  try {
    ...
    return {
      ireID:    item.uniqueID,
      id:       item.uniqueID,
      status:   item.status === 'leased' ? 'leased' : 'rent',
...
function parseResidential(item) {
  try {
    const status   = item.status === 'rent' ? 'rent' : item.status === 'sold' ? 'sold' : 'sale';
    const address  = item.address;
    const features = item.features || {};
    // Use soldDate for sold listings, dateAvailable for others
    const available = item.soldDetails?.soldDate || item.soldDate || item.dateAvailable || '';
    return {
      ireID:    item.uniqueID,
      id:       item.uniqueID,
      status,
