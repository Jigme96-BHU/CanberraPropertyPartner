...
export async function parseREAXML(xmlString) {
  ...
  try {
    ...
    // <rental> — current → "rent", leased → "leased", withdrawn → "withdrawn"
    ...
    // <residential> — current → "sale", sold → "sold", withdrawn → "withdrawn"
...
function buildAddress(address) {
  ...
  const streetNumber = typeof address.streetNumber === 'object' ? address.streetNumber._ : address.streetNumber;
...
// xml2js with mergeAttrs returns XML attributes merged into the object.
// So <rent period="weekly" display="yes">900</rent> becomes
// { _: "900", period: "weekly", display: "yes" }
// We always extract the text content from _ first, then fall back
// to the raw value if it's already a plain string/number.
// This handles ALL variations across the 18 listings safely.
...
function formatSalePrice(priceValue) {
  ...
  const val = extractValue(priceValue) || priceValue;
  const num = parseInt(val);
  if (isNaN(num)) return priceValue;
  return `$${num.toLocaleString()}`;
}

// Get property type from <category name="House">
// xml2js with mergeAttrs gives us { name: "House" }
...
function parseRental(item) {
  try {
    // withdrawn listings are removed from master by the cron
    const xmlStatus = item.status;
    let status;
    if (xmlStatus === 'current')   status = 'rent';
    else if (xmlStatus === 'leased')    status = 'leased';
    else if (xmlStatus === 'withdrawn') status = 'withdrawn';
    else return null; // ignore unknown statuses
    ...
    return {
      ...
      desc:     item.description || item.headline || '',
...
function parseResidential(item) {
  try {
    const xmlStatus = item.status;
    let status;
    if (xmlStatus === 'current')        status = item.status === 'rent' ? 'rent' : 'sale';
    else if (xmlStatus === 'sold')      status = 'sold';
    else if (xmlStatus === 'withdrawn') status = 'withdrawn';
    else return null; // ignore unknown statuses
    ...
    return {
      ...
      desc:     item.description || item.headline || '',
      headline: item.headline || '',
      soldDate: status === 'sold' ? (item.soldDetails?.date || '') : undefined,
    };
  } catch (e) {
