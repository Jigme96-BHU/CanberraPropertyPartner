...
export async function getListings() {
  ...
  try {
      ...
      if (newXml.length > 0) {
        ...
        for (const file of newXml) {
          try {
            ...
            const listings = await parseREAXML(xml, { currentOnly: true });
    ...
    // ── Step 3: Return active listings only ───────────────────
    const active = Object.values(master).filter(l => l.status === 'rent' || l.status === 'sale' || l.status === 'leased' || l.status === 'sold');
